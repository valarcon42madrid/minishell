...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/21 18:04:41 by valarcon          #+#    #+#             */
/*   Updated: 2022/11/29 10:48:08 by valarcon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/libminishell.h"

static void	close_fd_parent(t_process *process, t_process	*process_pid)
{
	ft_closerparent(&process_pid, &process);
	signal(SIGQUIT, signal_child2);
	signal(SIGINT, signal_child);
}

static void	redirect_input(t_shell *shell, int *fd_back, t_process *process_pid)
{
	int	fd_file;
...
static void	ft_c_r(t_shell *s, t_process *pr, int *fd_b, t_process *p_pid)
{
	ft_closer(&p_pid, &pr);
	pr->fd_out = get_fd_builtins(s);
	redirect_input(s, fd_b, p_pid);
	redirect_output(s, pr);
}

void	exec_final_child(t_shell *s, t_process *pr, int *fd_b, t_process *p_pid)
{
	char	*path;
